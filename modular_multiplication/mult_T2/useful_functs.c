...
void from_int_to_amns(int64_t *rop, mpz_t op){
	...
	while(tmp->_mp_size && (i < NB_COEFF)){
		...
		mpz_tdiv_q_2exp (tmp, tmp, BETA_LOG2);
	}
	...
	add_poly(rop, rop, rand_zero); // last step: + J
...
void init_data(){
	...
	mpz_set_str (modul_p, "87798663188023528073030994638480388226916752551484470548433013409422826400553", 10);

	mpz_set_str (gama_pow[0], "70543026063874968083330835012161440042437135154375017765226926371162653011399", 10);
