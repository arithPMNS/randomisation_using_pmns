...
void init_curve_and_base_point(int64_t *rand_pol){
	...
	mpz_set_ui(crv_b, 5);
	from_int_to_amns(curve_a, crv_a, rand_pol); // note : crv_a is set to 0 with the initialisation above; note: conv to mont_domain is done here 
	from_int_to_amns(curve_b, crv_b, rand_pol);  // note: conv to mont_domain is done here 
...
void from_aff_to_jacOneZ_toMontDomain(Inter_Point *jp, affPoint *ap, int64_t *rand_pol){
	from_int_to_amns(jp->X, ap->x, rand_pol);
	from_int_to_amns(jp->Y, ap->y, rand_pol);
}
...
void from_aff_to_jac_toMontDomain(JacPoint *jp, affPoint *ap, int64_t *rand_pol){
	from_int_to_amns(jp->X, ap->x, rand_pol);
	from_int_to_amns(jp->Y, ap->y, rand_pol);
	from_int_to_amns(jp->Z, const_ONE, rand_pol); // computation of a (random) representation of 'phi'
}
...
void dblu_with_free_adds__fixed_randPoly(int64_t *Z_out, Inter_Point *R, Inter_Point *S, Inter_Point *P, int64_t *rand_pol){
	...
	square_mod_poly(B, P->X, rand_pol);
	square_mod_poly(E, P->Y, rand_pol);
	square_mod_poly(L, E, rand_pol);
	
	lshift_poly_coeffs(S->Y, L, 3);
	exact_coeffs_reduction(S->Y, S->Y); // first
	
	add_poly(E, E, P->X); 
	square_mod_poly(E, E, rand_pol);
	sub_poly(E, E, B); 
	sub_poly(E, E, L); 
	...
	square_mod_poly(R->X, B, rand_pol);
	sub_poly(R->X, R->X, E); 
	
	sub_poly(L, S->X, R->X); 
	mult_mod_poly(R->Y, L, B, rand_pol);
...
void zaddu_with_free_adds__fixed_randPoly(Inter_Point *R, Inter_Point *S, Inter_Point *P, Inter_Point *Q, int64_t *Z_in, int64_t *Z_out, int64_t *rand_pol){
	...
	mult_mod_poly(Z_out, Z_in, C, rand_pol);
	
	square_mod_poly(C, C, rand_pol);
	mult_mod_poly(S->X, P->X, C, rand_pol); // W1
	mult_mod_poly(W, Q->X, C, rand_pol);    // W2
	
	sub_poly(C, P->Y, Q->Y);
	square_mod_poly(D, C, rand_pol);
	
	sub_poly(R->X, D, S->X);
	sub_poly(R->X, R->X, W);
	
	sub_poly(W, S->X, W);
	mult_mod_poly(S->Y, P->Y, W, rand_pol);
	
	sub_poly(D, S->X, R->X);
	mult_mod_poly(R->Y, D, C, rand_pol);
	sub_poly(R->Y, R->Y, S->Y);
}
...
void zaddc_with_free_adds__fixed_randPoly(Inter_Point *R, Inter_Point *S, Inter_Point *P, Inter_Point *Q, int64_t *Z_in, int64_t *Z_out, int64_t *rand_pol){
	...
	square_mod_poly(C, C, rand_pol);
	mult_mod_poly(W1, P->X, C, rand_pol);
	mult_mod_poly(W2, Q->X, C, rand_pol);
	
	sub_poly(C, P->Y, Q->Y);
	square_mod_poly(D1, C, rand_pol);
	
	sub_poly(R->X, D1, W1);
	sub_poly(R->X, R->X, W2); // X3
	
	sub_poly(D1, W1, W2);
	mult_mod_poly(D1, P->Y, D1, rand_pol); // A1
	
	
	add_poly(D2, P->Y, Q->Y);  // Y1 + Y2
	
	sub_poly(R->Y, W1, R->X);
	mult_mod_poly(R->Y, R->Y, C, rand_pol);
	sub_poly(R->Y, R->Y, D1);     // Y3
	
	square_mod_poly(S->X, D2, rand_pol); 
	sub_poly(S->X, S->X, W1);
	sub_poly(S->X, S->X, W2);   // X4
	
	sub_poly(S->Y, W1, S->X);
	mult_mod_poly(S->Y, S->Y, D2, rand_pol);
...
void coZ_montgomery_scal_mult_with_free_adds__fixed_randPoly(JacPoint *rop_Q, Inter_Point *op_P, char *scal_bits, int64_t *rand_pol){
	...
	dblu_with_free_adds__fixed_randPoly(tmp_coZ_points.Z, &(tmp_coZ_points.R[1]), &(tmp_coZ_points.R[0]), op_P, rand_pol);
	
	for(i=(SCAL_SIZE-2); i>=0; i--){
		b = scal_bits[i];
		
		zaddc_with_free_adds__fixed_randPoly(&(tmp_coZ_points.R[1-b]), &(tmp_coZ_points.R[b]), &(tmp_coZ_points.R[b]), &(tmp_coZ_points.R[1-b]), tmp_coZ_points.Z, tmp_coZ_points.Z, rand_pol);
		
		zaddu_with_free_adds__fixed_randPoly(&(tmp_coZ_points.R[b]), &(tmp_coZ_points.R[1-b]), &(tmp_coZ_points.R[1-b]), &(tmp_coZ_points.R[b]), tmp_coZ_points.Z, tmp_coZ_points.Z, rand_pol);
	}
