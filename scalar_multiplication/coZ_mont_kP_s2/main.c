...
int main(void){
	...
	//~ this 'rand_pol' will be used for conversion and all ECSM
	gen_rand_pol(rand_pol);
	

	init_data(); // for the AMNS
	init_curve_and_base_point(rand_pol); // for the curve
	init_affPoint(&aff_Q);
	...
	for(int i=0;i<NTEST;i++){
		from_aff_to_jacOneZ_toMontDomain(&jac_P, &aff_P, rand_pol);  // conversion step
		coZ_montgomery_scal_mult_with_free_adds__fixed_randPoly(&jac_Q, &jac_P, scal_k_bits[i], rand_pol);
	}
	...
	for(int i=0;i<NSAMPLES;i++){
		...
		for(int j=0;j<NTEST;j++){
			...
			from_aff_to_jacOneZ_toMontDomain(&jac_P, &aff_P, rand_pol); // conversion step
			coZ_montgomery_scal_mult_with_free_adds__fixed_randPoly(&jac_Q, &jac_P, scal_k_bits[j], rand_pol);
