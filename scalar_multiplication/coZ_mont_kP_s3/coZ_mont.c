...
void dblu_with_free_adds__fixed_randPoly(int64_t *Z_out, Inter_Point *R, Inter_Point *S, Inter_Point *P, int64_t *rand_zero){
	
	int64_t B[NB_COEFF];
	int64_t E[NB_COEFF];
	int64_t L[NB_COEFF]; 
	
	square_mod_poly(B, P->X, rand_zero);
	square_mod_poly(E, P->Y, rand_zero);
	square_mod_poly(L, E, rand_zero);
	
	lshift_poly_coeffs(S->Y, L, 3);
	exact_coeffs_reduction(S->Y, S->Y); // first
	
	add_poly(E, E, P->X); 
	square_mod_poly(E, E, rand_zero);
	sub_poly(E, E, B); 
	sub_poly(E, E, L); 
	...
	square_mod_poly(R->X, B, rand_zero);
	sub_poly(R->X, R->X, E); 
	
	sub_poly(L, S->X, R->X); 
	mult_mod_poly(R->Y, L, B, rand_zero);
...
void zaddu_with_free_adds__fixed_randPoly(Inter_Point *R, Inter_Point *S, Inter_Point *P, Inter_Point *Q, int64_t *Z_in, int64_t *Z_out, int64_t *rand_zero){
	...
	mult_mod_poly(Z_out, Z_in, C, rand_zero);
	
	square_mod_poly(C, C, rand_zero);
	mult_mod_poly(S->X, P->X, C, rand_zero); // W1
	mult_mod_poly(W, Q->X, C, rand_zero);    // W2
	
	sub_poly(C, P->Y, Q->Y);
	square_mod_poly(D, C, rand_zero);
	
	sub_poly(R->X, D, S->X);
	sub_poly(R->X, R->X, W);
	
	sub_poly(W, S->X, W);
	mult_mod_poly(S->Y, P->Y, W, rand_zero);
	
	sub_poly(D, S->X, R->X);
	mult_mod_poly(R->Y, D, C, rand_zero);
	sub_poly(R->Y, R->Y, S->Y);
}
...
//~ Note: the sequence of instructions allows inputs and outputs to be the same.
void zaddc_with_free_adds__fixed_randPoly(Inter_Point *R, Inter_Point *S, Inter_Point *P, Inter_Point *Q, int64_t *Z_in, int64_t *Z_out, int64_t *rand_zero){
	...
	square_mod_poly(C, C, rand_zero);
	mult_mod_poly(W1, P->X, C, rand_zero);
	mult_mod_poly(W2, Q->X, C, rand_zero);
	
	sub_poly(C, P->Y, Q->Y);
	square_mod_poly(D1, C, rand_zero);
	
	sub_poly(R->X, D1, W1);
	sub_poly(R->X, R->X, W2); // X3
	
	sub_poly(D1, W1, W2);
	mult_mod_poly(D1, P->Y, D1, rand_zero); // A1
	
	
	add_poly(D2, P->Y, Q->Y);  // Y1 + Y2
	
	sub_poly(R->Y, W1, R->X);
	mult_mod_poly(R->Y, R->Y, C, rand_zero);
	sub_poly(R->Y, R->Y, D1);     // Y3
	
	square_mod_poly(S->X, D2, rand_zero); 
	sub_poly(S->X, S->X, W1);
	sub_poly(S->X, S->X, W2);   // X4
	
	sub_poly(S->Y, W1, S->X);
	mult_mod_poly(S->Y, S->Y, D2, rand_zero);
...
void coZ_montgomery_scal_mult_with_free_adds__fixed_randPoly(JacPoint *rop_Q, Inter_Point *op_P, char *scal_bits){
	...
	dblu_with_free_adds__fixed_randPoly(tmp_coZ_points.Z, &(tmp_coZ_points.R[1]), &(tmp_coZ_points.R[0]), op_P, rand_zero);
	...
	for(i=(SCAL_SIZE-2); i>=0; i--){
		...
		zaddc_with_free_adds__fixed_randPoly(&(tmp_coZ_points.R[1-b]), &(tmp_coZ_points.R[b]), &(tmp_coZ_points.R[b]), &(tmp_coZ_points.R[1-b]), tmp_coZ_points.Z, tmp_coZ_points.Z, rand_zero);
		
		zaddu_with_free_adds__fixed_randPoly(&(tmp_coZ_points.R[b]), &(tmp_coZ_points.R[1-b]), &(tmp_coZ_points.R[1-b]), &(tmp_coZ_points.R[b]), tmp_coZ_points.Z, tmp_coZ_points.Z, rand_zero);
	}
