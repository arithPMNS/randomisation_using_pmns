...
int main(void){
	...
	for(int i=0;i<NTEST;i++){
		...
		coZ_montgomery_scal_mult_with_free_adds__fixed_randPoly(&jac_Q, &jac_P, scal_k_bits[i]);
	}
	...
	for(int i=0;i<NSAMPLES;i++){
		...
		for(int j=0;j<NTEST;j++){
			...
			from_aff_to_jacOneZ_toMontDomain(&jac_P, &aff_P); // conversion step
			coZ_montgomery_scal_mult_with_free_adds__fixed_randPoly(&jac_Q, &jac_P, scal_k_bits[j]);
